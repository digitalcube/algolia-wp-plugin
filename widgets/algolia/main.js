if (jQuery("#algolia-search-box, #algolia-object").length > 0) {
  // Algolia Config
  document.addEventListener("DOMContentLoaded", function (event) {
    ...
    const localeName = settings.localeName
      ...
      : ``;
    const facets = settings.facetFilters
      ? JSON.parse(settings.facetFilters)
      : ``;
    ...
    if (jQuery("#algolia-stats").length > 0) {
      ...
      const statsWithPanel = instantsearch.widgets.panel({
        templates: {
          header: `${
            statsSettings.panelHeader ? statsSettings.panelHeader : ``
          }`,
        },
